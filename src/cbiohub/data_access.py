...
def get_local_combined_data_from_folders(
    folder, type: Literal["sample", "patient", "mutations"]
):
    ...
    for f in tqdm(files):
        ...
        if os.path.exists(parquet_filename) and os.path.getmtime(
            parquet_filename
        ) >= os.path.getmtime(f):
            ...
            try:
                df = pd.read_csv(
                    f, sep="\t", comment="#", low_memory=False, dtype=dtype
                )
