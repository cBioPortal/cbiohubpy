...
def ingest(folder_name):
    ...
    if folder_path.is_dir():
        ...
        study_paths = [p for p in folder_path.iterdir() if p.is_dir() and Study.is_study(p)]
        if not study_paths:
            # Single study
        ...
        with tqdm(total=len(study_paths), desc="Processing studies", unit="study") as pbar:
            for study_path in study_paths:
                ...
                if study.check_integrity():
                    if study.create_parquets():
                        ...
                        click.echo(f"⚠️ Skipped study {study.name} due to errors during Parquet creation.")
                        skipped_due_to_errors_count += 1
                        skipped_due_to_errors_studies.append(study.name)
                else:
                    click.echo(f"⚠️ Skipped study {study.name} due to missing required files.")
                    skipped_due_to_missing_files_count += 1
                    skipped_due_to_missing_files_studies.append(study.name)
                pbar.update(1)

        click.echo(click.style(f"✅ Finished processing {processed_count} studies.", fg='green'))
        click.echo(click.style(f"ℹ️ {already_processed_count} studies were already processed."))
        click.echo(click.style(f"⚠️ {skipped_due_to_errors_count} studies were skipped due to errors during Parquet creation.", fg='red'))
        if skipped_due_to_errors_studies:
            click.echo(click.style(f"Skipped due to errors: {', '.join(skipped_due_to_errors_studies)}", fg='red'))
        click.echo(click.style(f"⚠️ {skipped_due_to_missing_files_count} studies were skipped due to missing required files.", fg='yellow'))
        if skipped_due_to_missing_files_studies:
            click.echo(click.style(f"Skipped due to missing files: {', '.join(skipped_due_to_missing_files_studies)}", fg='yellow'))
    else:
...
def combine(output_dir):
    ...
    with tqdm(total=len(study_paths), desc="Loading studies", unit="study") as pbar:
        for study_path in processed_studies_path.iterdir():
            if study_path.is_dir():
                ...
                clinical_patient_file = study.processed_path / 'data_clinical_patient.parquet'
    ...
    if mutation_tables:
        ...
        click.echo(click.style(f"⏱️ Concatenation time: {concat_time} seconds", fg='green'))

        start_time = time.time()
        pq.write_table(combined_mutations, combined_path / 'combined_mutations.parquet')
        write_time = time.time() - start_time

        click.echo(click.style(f"✅ Combined mutations saved to {combined_path / 'combined_mutations.parquet'}", fg='green'))
        click.echo(click.style(f"⏱️ Write time: {write_time} seconds", fg='green'))

    if clinical_patient_tables:
        start_time = time.time()
        combined_clinical_patient = pa.concat_tables(clinical_patient_tables, promote=True)
        concat_time = time.time() - start_time

        click.echo(click.style(f"⏱️ Concatenation time: {concat_time} seconds", fg='green'))

        start_time = time.time()
        pq.write_table(combined_clinical_patient, combined_path / 'combined_clinical_patient.parquet')
        write_time = time.time() - start_time

        click.echo(click.style(f"✅ Combined clinical patient data saved to {combined_path / 'combined_clinical_patient.parquet'}", fg='green'))
        click.echo(click.style(f"⏱️ Write time: {write_time} seconds", fg='green'))

    if clinical_sample_tables:
        start_time = time.time()
        combined_clinical_sample = pa.concat_tables(clinical_sample_tables, promote=True)
        concat_time = time.time() - start_time

        click.echo(click.style(f"⏱️ Concatenation time: {concat_time} seconds", fg='green'))

        start_time = time.time()
        pq.write_table(combined_clinical_sample, combined_path / 'combined_clinical_sample.parquet')
        write_time = time.time() - start_time

        click.echo(click.style(f"✅ Combined clinical sample data saved to {combined_path / 'combined_clinical_sample.parquet'}", fg='green'))
        click.echo(click.style(f"⏱️ Write time: {write_time} seconds", fg='green'))
