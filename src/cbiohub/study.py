...
class Study:
    ...
    def check_integrity(self):
        ...
        missing_files = [file_name for file_name in required_files if not (self.study_path / file_name).exists()]
        if missing_files:
            print(f"Study {self.name} is missing the following files: {', '.join(missing_files)}")
            return False
        return True
    ...
    def list_files(self, file_type: str):
        ...
        if not file_path.exists():
            raise FileNotFoundError(f"File {file_name} not found in study {self.study_path}.")
    ...
    def create_parquet(self, file_type: str):
        ...
        try:
            ...
            df = pd.read_csv(file_path, sep='\t', comment='#', low_memory=False, dtype=str)
    ...
    def is_processed(self):
        ...
        for source_file in source_files:
            if source_file.exists() and source_file.stat().st_mtime > success_file_mtime:
                return False
